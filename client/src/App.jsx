...
function App() {
  ...
  useEffect(() => {
    const showProducts = async () => {
      try {
        const response = await fetch(process.env.REACT_APP_SERVER_URL + "/products/show-products");
        if(!response.ok){
    ...
    <div className={modeClass}>
      ...
      <Header products={products} toggleMode={toggleMode} />
        <Routes>
          ...
          <Route path="/iPhone" element={<ApplePhone products={products} />} />
          ...
          <Route path="/product-about/:productCategory/:productName" element={<ProductPage products={products} />} />
        </Routes>
        <Footer />
      </BrowserRouter>
    </div>
